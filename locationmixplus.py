            ...
            for word in query:
                if word == 'and' or word == 'or' or word == 'not' or word == '('or word == ')':
                    ...
                    if len(query_words) == 1:
                        ...
                        for line in content:
                            my_list.append(line.strip())
                        if all(word in inverted_index for word in query_words):
                            common_docs = set(inverted_index[query_words[0]][1].keys())
                            for word in query_words[1:]:
                                common_docs.intersection_update(inverted_index[word][1].keys())
                            for doc_id in common_docs:
                                positions_list = [inverted_index[word][1][doc_id]['positions'] for word in query_words]
                                found_positions = []
                                for positions in positions_list[0]:
                                    for i in range(1, len(positions_list)):
                                        if positions + i not in positions_list[i]:
                                            break
                                    else:
...
if __name__ == "__main__":
    with open("daopaisuoyinweizhi.txt", 'r', encoding='utf-8') as file:
        content = file.read()
